// Port of hyper_tungstenite for fastwebsockets.
...
use tokio_uring::io::{self, AsyncWriteExt};
...
#[async_trait]
impl<S> FromRequestParts<S> for IncomingUpgrade
where
    S: Sync,
{
...
/// this function returns true if one of them is "websocket".
...
impl std::future::Future for UpgradeFut {
    type Output = Result<WebSocket<TcpStream>, Error>;

    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let this = self.project();
        let upgraded = match this.inner.poll_unpin(cx) {
            Poll::Pending => return Poll::Pending,
            Poll::Ready(x) => x,
        };
        let upgraded = upgraded.map_err(|_| WebSocketError::UpgradeError)?;

        // Convert the hyper Upgraded to tokio_uring TcpStream
        let tcp_stream = TcpStream::from(upgraded);
